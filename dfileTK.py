# dfileTK - Tool kit for work with d-file
# Martin Barton 2020 FBMI CVUT
# ma.barton@seznam.cz
...
class DFile:
    ...
    def preprocess_flow(self):
        ...
        # Segmentace
        seg_len_sec = 5  # Delka segmentu v sec
    ...
    def preprocess_flow_slide(self):
        ...
        # Nacteni dat
        self.data_load()  # Nacte data
        spo2_orig = self.chan_data("flow")  # Nacteni jednoho kanalu (spo2)
    ...
    def preprocess_spo2_slide(self):
        ...
        # Urceni pritomnisti apnoe v segmentu
        tags = self.istag_slide([("S+", "S-")], spo2_seg, times)  # Udela vektor s oznacenim segmentu
        if tags == []:
            raise Exception("Tags problem!")
...
def compare2tag_sets(orig_tag_s, orig_tag_e, new_tag):
    ...
    print("PPV:\t"         + str(round(((TP) / (TP + FP)) * 100, 2)) + " %")
    print("FDR:\t"         + str(round(((FP) / (FP + TP)) * 100, 2)) + " %")
